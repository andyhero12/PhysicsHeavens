 ...
 */
std::shared_ptr<NetEvent> GameResEvent::newEvent(){
    return std::make_shared<GameResEvent>();
}

std::shared_ptr<NetEvent> GameResEvent::allocGameResEvent(Vec2 pos){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<GameResEvent>();
    event->_pos = pos;
    return event;
 ...
 */
std::vector<std::byte> GameResEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void GameResEvent::deserialize(const std::vector<std::byte>& data){
