 ...
 */
std::shared_ptr<NetEvent> ShootEvent::newEvent(){
    return std::make_shared<ShootEvent>();
}

std::shared_ptr<NetEvent> ShootEvent::allocShootEvent(Vec2 pos, float ang, float scale , bool isHost){
 ...
 */
std::vector<std::byte> ShootEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void ShootEvent::deserialize(const std::vector<std::byte>& data){
