 ...
 */
std::shared_ptr<NetEvent> DecoyEvent::newEvent(){
    return std::make_shared<DecoyEvent>();
}

std::shared_ptr<NetEvent> DecoyEvent::allocDecoyEvent(Vec2 pos){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<DecoyEvent>();
    event->_pos = pos;
    return event;
 ...
 */
std::vector<std::byte> DecoyEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void DecoyEvent::deserialize(const std::vector<std::byte>& data){
