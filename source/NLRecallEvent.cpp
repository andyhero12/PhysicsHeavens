 ...
 */
std::shared_ptr<NetEvent> RecallEvent::newEvent(){
    return std::make_shared<RecallEvent>();
}

std::shared_ptr<NetEvent> RecallEvent::allocRecallEvent(Vec2 pos, bool isHost){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<RecallEvent>();
    event->_pos = pos;
 ...
 */
std::vector<std::byte> RecallEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void RecallEvent::deserialize(const std::vector<std::byte>& data){
