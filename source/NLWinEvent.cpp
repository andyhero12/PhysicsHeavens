 ...
 */
std::shared_ptr<NetEvent> WinEvent::newEvent(){
    return std::make_shared<WinEvent>();
}

std::shared_ptr<NetEvent> WinEvent::allocWinEvent(Vec2 pos, bool isHost){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<WinEvent>();
    event->_pos = pos;
 ...
 */
std::vector<std::byte> WinEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void WinEvent::deserialize(const std::vector<std::byte>& data){
