 ...
 */
std::shared_ptr<NetEvent> SpawnerDeathEvent::newEvent(){
    return std::make_shared<SpawnerDeathEvent>();
}

std::shared_ptr<NetEvent> SpawnerDeathEvent::allocSpawnerDeathEvent(Vec2 pos){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<SpawnerDeathEvent>();
    event->_pos = pos;
    return event;
 ...
 */
std::vector<std::byte> SpawnerDeathEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void SpawnerDeathEvent::deserialize(const std::vector<std::byte>& data){
