 ...
 */
std::shared_ptr<NetEvent> ExplodeEvent::newEvent(){
    return std::make_shared<ExplodeEvent>();
}

std::shared_ptr<NetEvent> ExplodeEvent::allocExplodeEvent(Vec2 pos){
 ...
 */
std::vector<std::byte> ExplodeEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void ExplodeEvent::deserialize(const std::vector<std::byte>& data){
