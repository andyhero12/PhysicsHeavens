 ...
 */
std::shared_ptr<NetEvent> LoseEvent::newEvent(){
    return std::make_shared<LoseEvent>();
}

std::shared_ptr<NetEvent> LoseEvent::allocLoseEvent(Vec2 pos, bool isHost){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<LoseEvent>();
    event->_pos = pos;
 ...
 */
std::vector<std::byte> LoseEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
 ...
 */
void LoseEvent::deserialize(const std::vector<std::byte>& data){
