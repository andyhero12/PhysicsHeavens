 ...
 */
std::shared_ptr<NetEvent> ClientHealthEvent::newEvent(){
    return std::make_shared<ClientHealthEvent>();
}

std::shared_ptr<NetEvent> ClientHealthEvent::allocClientHealthEvent(int healthLost, bool m_isHost){
    //TODO: make a new shared copy of the event and set its _pos to pos.
#pragma mark BEGIN SOLUTION
    auto event = std::make_shared<ClientHealthEvent>();
    event->_health = healthLost;
    event->_hostDog = m_isHost;
    return event;
 ...
 */
std::vector<std::byte> ClientHealthEvent::serialize(){
    //TODO: serialize _pos
#pragma mark BEGIN SOLUTION
    _serializer.reset();
    _serializer.writeSint32(_health);
    _serializer.writeBool(_hostDog);
 ...
 */
void ClientHealthEvent::deserialize(const std::vector<std::byte>& data){
